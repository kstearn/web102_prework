...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        newDiv.innerHTML = `
                ...
                <h5>$${game.pledged.toLocaleString('en-US')} raised / $${game.goal.toLocaleString('en-US')} goal</h5>
                `;
            
        if (game.pledged > game.goal) {
            newDiv.innerHTML += `<meter min='0' max='100' value='100'></meter>`;
        } else {
            newDiv.innerHTML += `<meter min='0' max='${game.goal}' value='${game.pledged}'></meter>`;
        }
...
function filterUnfundedOnly() {
    ...
    const notMetList = GAMES_JSON.filter(game => {
        return game.pledged < game.goal;
    })

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(notMetList);
}
...
function filterFundedOnly() {
    ...
    const metGoalList = GAMES_JSON.filter(game => {
        return game.pledged > game.goal;
    })
...
const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} ${totalGames > 1 ? "games" : "game"}. Currently, ${numUnfunded} ${numUnfunded > 1 ? "games remain" : "game remains"} unfunded. We need your help to fund these amazing games!`;
